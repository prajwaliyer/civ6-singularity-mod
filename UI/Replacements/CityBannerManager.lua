...
-- Singularity: Inorganic Life power-based growth
local SINGULARITY_POWER_PER_CITIZEN			:number = 2;
local SINGULARITY_DATA_CENTER_POWER_COST	:number = 10;
local SINGULARITY_DISTRICT_DATA_CENTER		= GameInfo.Districts["DISTRICT_DATA_CENTER"];

function Singularity_CountDataCentersInCity(pCity)
	if SINGULARITY_DISTRICT_DATA_CENTER == nil then return 0 end
	local count = 0
	...
	if cityPlots then
		for _, plotIdx in ipairs(cityPlots) do
			...
			if pPlot and pPlot:GetDistrictType() == SINGULARITY_DISTRICT_DATA_CENTER.Index then
				count = count + 1
			end
		end
	end
...
function Singularity_GetCityPowerSurplus(pCity)
	local pPower = pCity:GetPower()
	...
	local popConsumption = pCity:GetPopulation() * SINGULARITY_POWER_PER_CITIZEN
	local dcPowerDraw = Singularity_CountDataCentersInCity(pCity) * SINGULARITY_DATA_CENTER_POWER_COST
...
function CityBanner:UpdatePopulation(isLocalPlayer:boolean, pCity:table, pCityGrowth:table)
	...
	if isLocalPlayer then
		-- Singularity: Check if city is converted to Inorganic Life
		local isInorganic:boolean = (pCity:GetProperty("INORGANIC_LIFE") == 1);

		if isInorganic then
			-- Power-based growth for Inorganic Life cities
			local powerSurplus:number = Singularity_GetCityPowerSurplus(pCity);
			local accumulated:number = pCity:GetProperty("POWER_GROWTH") or 0;
			local growthThreshold:number = pCityGrowth:GetGrowthThreshold();
			local powerpct:number = Clamp( accumulated / growthThreshold, 0.0, 1.0 );

			local turnsUntilGrowth:number = 0;
			if powerSurplus > 0 then
				local remaining = growthThreshold - accumulated;
				if remaining <= 0 then
					turnsUntilGrowth = 1;
				else
					turnsUntilGrowth = math.ceil(remaining / powerSurplus);
				end
			elseif powerSurplus < 0 then
				turnsUntilGrowth = -1; -- declining
			end

			local popTooltip:string = "[ICON_Citizen] Population: " .. currentPopulation;
			if turnsUntilGrowth > 0 then
				popTooltip = popTooltip .. "[NEWLINE]  " .. Locale.Lookup("LOC_CITY_BANNER_TURNS_GROWTH", turnsUntilGrowth);
				popTooltip = popTooltip .. "[NEWLINE]  " .. toPlusMinusString(powerSurplus) .. " surplus [ICON_Power] Power.";
			elseif turnsUntilGrowth == 0 then
				popTooltip = popTooltip .. "[NEWLINE]  " .. Locale.Lookup("LOC_CITY_BANNER_STAGNATE");
			elseif turnsUntilGrowth < 0 then
				popTooltip = popTooltip .. "[NEWLINE]  " .. Locale.Lookup("LOC_CITY_BANNER_TURNS_STARVATION", 1);
				popTooltip = popTooltip .. "[NEWLINE]  " .. toPlusMinusString(powerSurplus) .. " deficit [ICON_Power] Power.";
			end
			populationInstance.FillMeter:SetToolTipString(popTooltip);
			if turnsUntilGrowth > 0 then
				populationInstance.CityPopTurnsLeft:SetText(turnsUntilGrowth);
			elseif turnsUntilGrowth < 0 then
				populationInstance.CityPopTurnsLeft:SetText("[COLOR_RED]-1[ENDCOLOR]");
			else
				populationInstance.CityPopTurnsLeft:SetText("-");
			end

			populationInstance.FillMeter:SetPercent(powerpct);
			populationInstance.SlotMeter:SetPercent(1 - powerpct);
		else
			-- Normal food-based growth
