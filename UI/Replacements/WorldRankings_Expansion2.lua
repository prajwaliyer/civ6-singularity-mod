...
-- Modified by Singularity mod: replaces Exoplanet Expedition with Dyson Swarm (6 bars)
...
-- Base game projects (safe to look up at load time)
...
-- Singularity: Bar 4 = Research Offworld Mission + Build Dyson Swarm
-- DEFERRED lookup: custom mod projects may not be in GameInfo at file load time.
local DYSON_BUILD_PROJECT_INFOS = nil;
function GetDysonBuildProjectInfos()
	if DYSON_BUILD_PROJECT_INFOS == nil then
		DYSON_BUILD_PROJECT_INFOS = {};
		local info = GameInfo.Projects["PROJECT_BUILD_DYSON_SWARM"];
		if info ~= nil then
			table.insert(DYSON_BUILD_PROJECT_INFOS, info);
		end
	end
	return DYSON_BUILD_PROJECT_INFOS;
end

-- Singularity: Bar 5 = Launch Q1-Q4 + Complete Dyson Swarm (no tech prereq shown)
local DYSON_LAUNCH_COMPLETE_PROJECT_INFOS = nil;
function GetDysonLaunchCompleteProjectInfos()
	if DYSON_LAUNCH_COMPLETE_PROJECT_INFOS == nil then
		DYSON_LAUNCH_COMPLETE_PROJECT_INFOS = {};
		local names = {
			"PROJECT_LAUNCH_DYSON_SWARM_Q1",
			"PROJECT_LAUNCH_DYSON_SWARM_Q2",
			"PROJECT_LAUNCH_DYSON_SWARM_Q3",
			"PROJECT_LAUNCH_DYSON_SWARM_Q4",
			"PROJECT_LAUNCH_EXOPLANET_EXPEDITION", -- renamed to "Complete Dyson Swarm"
		};
		for _, name in ipairs(names) do
			local info = GameInfo.Projects[name];
			if info ~= nil then
				table.insert(DYSON_LAUNCH_COMPLETE_PROJECT_INFOS, info);
			end
		end
	end
	return DYSON_LAUNCH_COMPLETE_PROJECT_INFOS;
end

-- Combined for GetNextStep / tooltip compatibility
local SCIENCE_PROJECTS_EXP2 = nil;
function GetScienceProjectsExp2()
	if SCIENCE_PROJECTS_EXP2 == nil then
		SCIENCE_PROJECTS_EXP2 = {
			EARTH_SATELLITE_EXP2_PROJECT_INFOS,
			MOON_LANDING_EXP2_PROJECT_INFOS,
			MARS_COLONY_EXP2_PROJECT_INFOS,
			GetDysonBuildProjectInfos(),
			GetDysonLaunchCompleteProjectInfos()
		};
	end
	return SCIENCE_PROJECTS_EXP2;
end
...
local SCIENCE_DETAILS:string = "To achieve a Science Victory you must accomplish five major milestones, then complete the Dyson Swarm to achieve technological transcendence.";
...
function ViewScience()
	...
	local scienceProjects = GetScienceProjectsExp2();
	local dysonBuildInfos = GetDysonBuildProjectInfos();
	local dysonLaunchCompleteInfos = GetDysonLaunchCompleteProjectInfos();

	local totalCost:number = 0;
	local currentProgress:number = 0;
	local progressText:string = "";
	local progressResults:table = { 0, 0, 0, 0, 0 };
	...
	if (g_LocalPlayer ~= nil) then
		...
		for _, city in pPlayerCities:Members() do
			...
			-- 4th milestone - Build Dyson Swarm
			totalCost = 0;
			currentProgress = 0;
			for i, projectInfo in ipairs(dysonBuildInfos) do
				local projectCost:number = pBuildQueue:GetProjectCost(projectInfo.Index);
				local projectProgress:number = projectCost;
				if pPlayerStats:GetNumProjectsAdvanced(projectInfo.Index) == 0 then
					projectProgress = pBuildQueue:GetProjectProgress(projectInfo.Index);
			...
			-- 5th milestone - Launch + Complete Dyson Swarm
			totalCost = 0;
			currentProgress = 0;
			for i, projectInfo in ipairs(dysonLaunchCompleteInfos) do
				local projectCost:number = pBuildQueue:GetProjectCost(projectInfo.Index);
				local projectProgress:number = projectCost;
				if pPlayerStats:GetNumProjectsAdvanced(projectInfo.Index) == 0 then
					projectProgress = pBuildQueue:GetProjectProgress(projectInfo.Index);
	...
	for i, result in ipairs(progressResults) do
		if(result < 1) then
			...
			if(nextStep == "") then
				nextStep = GetNextStepForScienceProject(g_LocalPlayer, scienceProjects[i], bHasSpaceport, finishedProjects[i]);
			end
		else
	...
	-- Final milestone - Earning Victory Points (bar 6)
	...
	progressText = progressText .. "Achieve Technological Transcendence";
    ...
    if (nextStep ~= "") then
		...
		local totalLY = g_LocalPlayer:GetStats():GetScienceVictoryPointsTotalNeeded();
		local curLY = g_LocalPlayer:GetStats():GetScienceVictoryPoints();
		local perTurnLY = g_LocalPlayer:GetStats():GetScienceVictoryPointsPerTurn();
		local turnsRemaining = 0;
		if perTurnLY > 0 then
			turnsRemaining = math.ceil((totalLY - curLY) / perTurnLY);
		end
		if turnsRemaining <= 0 then turnsRemaining = 1; end
		if turnsRemaining == 1 then
			g_activeheader.AdvisorTextNextStep:SetText("Your Dyson Swarm will reach the sun in 1 turn.");
		else
			g_activeheader.AdvisorTextNextStep:SetText("Your Dyson Swarm will reach the sun in " .. turnsRemaining .. " turns.");
		end
    end
...
function PopulateScienceInstance(instance:table, pPlayer:table)
	...
	local projectTotals:table = { 0, 0, 0, 0, 0 };
	local projectProgresses:table = { 0, 0, 0, 0, 0 };
	local finishedProjects:table = { {}, {}, {}, {}, {} };
	for _, city in pPlayerCities:Members() do
		local pBuildQueue:table = city:GetBuildQueue();
...
function GetTooltipForScienceProject(pPlayer:table, projectInfos:table, bHasSpaceport:boolean, finishedProjects:table, skipTechPrereqs:boolean)
	...
	local playerTech:table = pPlayer:GetTechs();
	local shownTechs:table = {};
	local numProjectInfos:number = table.count(projectInfos);
	for i, projectInfo in ipairs(projectInfos) do

		if(not skipTechPrereqs and projectInfo.PrereqTech ~= nil and not shownTechs[projectInfo.PrereqTech]) then
			shownTechs[projectInfo.PrereqTech] = true;
			local tech:table = GameInfo.Technologies[projectInfo.PrereqTech];
...
function PopulateScienceProgressMeters(instance:table, progressData:table)
	...
	-- Bar 6: VP / transcendence progress
	instance["ObjHidden_6"]:SetHide(true);
	if ((progressData.projectProgresses[5] >= progressData.projectTotals[5]) and (progressData.projectTotals[5] ~= 0)) then
		...
		instance["ObjFill_6"]:SetHide(false);
		instance["ObjToggle_ON_6"]:SetHide(lightYears == 0 or lightYears < lightYearsPerTurn);
		if lightYears > totalLightYears then
			lightYears = totalLightYears;
		end
		instance["ObjBar_6"]:SetPercent(lightYears/totalLightYears);
		local turnsLeft = 0;
		if lightYearsPerTurn > 0 then
			turnsLeft = math.ceil((totalLightYears - lightYears) / lightYearsPerTurn);
		end
		if turnsLeft <= 0 then turnsLeft = 1; end
		if turnsLeft == 1 then
			instance.ObjBG_6:SetToolTipString("Your Dyson Swarm will reach the sun in 1 turn.");
		else
			instance.ObjBG_6:SetToolTipString("Your Dyson Swarm will reach the sun in " .. turnsLeft .. " turns.");
		end
	else
		instance["ObjFill_6"]:SetHide(true);
		instance["ObjToggle_ON_6"]:SetHide(true);
		instance["ObjBar_6"]:SetPercent(0);
		instance.ObjBG_6:SetToolTipString("Dyson Swarm has not been completed.");
	end

	-- Tooltips for bars 1-5
	...
	instance.ObjBG_4:SetToolTipString(GetTooltipForScienceProject(pPlayer, dysonBuildInfos, nil, progressData.finishedProjects[4], false));
	instance.ObjBG_5:SetToolTipString(GetTooltipForScienceProject(pPlayer, dysonLaunchCompleteInfos, nil, progressData.finishedProjects[5], true));
end
